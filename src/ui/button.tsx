...
export default forwardRef<HTMLButtonElement, ButtonHTMLAttributes<HTMLButtonElement> & Props>(function Button(
  ...
  return (
    <button
      className={`relative border border-primary transition disabled:cursor-not-allowed ${className} ${
        kind === "primary" ? "bg-primary text-white" : "bg-transparent text-primary"
