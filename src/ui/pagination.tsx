...
function PageButton({
  ...
  return (
    <button
      className={`inline-flex h-8 w-8 items-center justify-center rounded-small border text-sm font-medium text-white transition disabled:scale-100 disabled:cursor-not-allowed disabled:border-transparent disabled:opacity-60 ${
        page === "more" || page === currentPage ? "hover:cursor-default" : "hover:border-primary active:scale-95"
      } ${page === currentPage ? "border-primary bg-primary" : "border-transparent bg-component"}`}
      onClick={onClick}
      disabled={disabled}
    >
      {page === "more" ? (
