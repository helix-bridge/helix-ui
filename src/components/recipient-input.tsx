...
export default function RecipientInput({ value, options = [], onChange = () => undefined }: Props) {
  ...
  return (
    <>
      <div
        className="group mx-medium flex items-center justify-between gap-small rounded-[0.625rem] bg-[#1F282C] px-medium py-2 transition-colors focus-within:bg-white/10 hover:bg-white/10"
        ref={refs.setReference}
        {...getReferenceProps()}
      >
        <div className="flex w-full items-center gap-small lg:gap-2">
          <WalletSVG
            width={20}
            height={20}
