...
export default function WithdrawableLiquiditiesSelect({
  ...
  return (
    <div className={`relative max-h-60 rounded-middle bg-inner ${loading ? "overflow-y-hidden" : "overflow-y-auto"}`}>
      <ComponentLoading loading={loading} />

      <div className="flex flex-col py-2">
        {options.length ? (
          options.map((option) => (
            <div key={option.id} className="flex items-center justify-between gap-middle px-large py-2">
      ...
      {total > options.length ? (
        <button
          onClick={onLoadMore}
          className="w-full rounded-b-middle border-t border-t-white/10 py-2 text-sm font-medium text-white transition-[transform,color] hover:bg-primary active:translate-y-1"
        >
          Load More
        </button>
      ) : null}
