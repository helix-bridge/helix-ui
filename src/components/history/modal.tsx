...
export default function Modal({ children, isDetail, isOpen, onBack, onClose }: PropsWithChildren<Props>) {
  ...
  return createPortal(
    ...
    >
      {/* Mask */}
      <div
        className="fixed left-0 top-0 z-[21] flex h-screen w-screen items-center justify-center overflow-auto bg-background/80 p-medium backdrop-blur-sm"
        onClick={(e) => e.stopPropagation()}
        ref={nodeRef}
      >
        {/* Modal */}
        <div className="relative h-[36rem] w-full rounded-3xl bg-secondary p-medium lg:w-[42rem] lg:p-5">
          {/* Close */}
          <button
