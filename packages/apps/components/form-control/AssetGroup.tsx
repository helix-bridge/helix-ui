...
type AssetGroupValue = (CrossChainAsset<DarwiniaAsset> & { checked?: boolean; decimals?: number })[];
...
export function AssetGroup({
  ...
  const feeFormatted = useMemo(() => {
    return fromWei({ value: fee, decimals: ringBalance?.token.decimals ?? 9 });
  }, [fee, ringBalance]);
      ...
      {(fields, _operations, { errors }) => (
        <>
          ...
          {fields?.map((field, index) => {
            ...
            const decimals = balance?.token.decimals || 9;
                  ...
                  >
                    <MaxBalance
                      ...
                      onClick={() => {
                        ...
                        const val = {
                          ...target,
                          amount: fromWei({ value: max, decimals }),
                        };
