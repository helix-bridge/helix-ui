...
module.exports = {
  ...
  fixBlankPage: async () => {
    ...
    for (let times = 0; times < 5; times++) {
      if ((await puppeteer.metamaskWindow().$(welcomePageElements.app)) === null) {
  ...
  importWallet: async (secretWords, password) => {
    ...
    await puppeteer.waitAndType(firstTimeFlowFormPageElements.secretWordsInput, secretWords);
    ...
    if ((await puppeteer.metamaskWindow().$(mainPageElements.popup.container)) !== null) {
  ...
  createWallet: async (password) => {
    ...
    await puppeteer.waitAndClick(metametricsPageElements.optOutAnalyticsButton);
    await puppeteer.waitAndType(firstTimeFlowFormPageElements.newPasswordInput, password);
    await puppeteer.waitAndType(firstTimeFlowFormPageElements.confirmPasswordInput, password);
  ...
  changeNetwork: async (network) => {
    ...
    if (network === 'main' || network === 'mainnet') {
      await puppeteer.waitAndClick(mainPageElements.networkSwitcher.networkButton(0));
    } else if (network === 'ropsten') {
      await puppeteer.waitAndClick(mainPageElements.networkSwitcher.networkButton(1));
    } else if (network === 'kovan') {
      await puppeteer.waitAndClick(mainPageElements.networkSwitcher.networkButton(2));
    } else if (network === 'rinkeby') {
      await puppeteer.waitAndClick(mainPageElements.networkSwitcher.networkButton(3));
    } else if (network === 'goerli') {
      await puppeteer.waitAndClick(mainPageElements.networkSwitcher.networkButton(4));
    } else if (network === 'localhost') {
      await puppeteer.waitAndClick(mainPageElements.networkSwitcher.networkButton(5));
    } else if (typeof network === 'object') {
      await puppeteer.waitAndClickByText(mainPageElements.networkSwitcher.dropdownMenuItem, network.networkName);
    } else {
  ...
  addNetwork: async (network) => {
    if (process.env.NETWORK_NAME && process.env.RPC_URL && process.env.CHAIN_ID) {
      network = {
  ...
  async disconnectWalletFromAllDapps() {
    ...
    await puppeteer.waitAndClick(mainPageElements.optionsMenu.button);
    await puppeteer.waitAndClick(mainPageElements.optionsMenu.connectedSitesButton);
    const trashButtons = await puppeteer.metamaskWindow().$$(mainPageElements.connectedSites.trashButton);
  ...
  activateCustomNonce: async () => {
    ...
    if ((await puppeteer.metamaskWindow().$(advancedPageElements.customNonceToggleOn)) === null) {
  ...
  confirmSignatureRequest: async () => {
    ...
    await puppeteer.waitAndClick(signaturePageElements.confirmSignatureRequestButton, notificationPage);
    await puppeteer.metamaskWindow().waitForTimeout(3000);
    return true;
  },
  rejectSignatureRequest: async () => {
    const notificationPage = await puppeteer.switchToMetamaskNotification();
    await puppeteer.waitAndClick(signaturePageElements.rejectSignatureRequestButton, notificationPage);
    await puppeteer.metamaskWindow().waitForTimeout(3000);
    return true;
  },
  confirmPermissionToSpend: async () => {
    const notificationPage = await puppeteer.switchToMetamaskNotification();
    await puppeteer.waitAndClick(notificationPageElements.allowToSpendButton, notificationPage);
    await puppeteer.metamaskWindow().waitForTimeout(3000);
    return true;
  },
  rejectPermissionToSpend: async () => {
    const notificationPage = await puppeteer.switchToMetamaskNotification();
    await puppeteer.waitAndClick(notificationPageElements.rejectToSpendButton, notificationPage);
    await puppeteer.metamaskWindow().waitForTimeout(3000);
    return true;
  },
  acceptAccess: async () => {
    const notificationPage = await puppeteer.switchToMetamaskNotification();
    await puppeteer.waitAndClick(notificationPageElements.nextButton, notificationPage);
  ...
  rejectTransaction: async () => {
    const notificationPage = await puppeteer.switchToMetamaskNotification();
    await puppeteer.waitAndClick(confirmPageElements.rejectButton, notificationPage);
    await puppeteer.metamaskWindow().waitForTimeout(3000);
    return true;
  },
  getWalletAddress: async () => {
    await puppeteer.waitAndClick(mainPageElements.optionsMenu.button);
    await puppeteer.waitAndClick(mainPageElements.optionsMenu.accountDetailsButton);
    walletAddress = await puppeteer.waitAndGetValue(mainPageElements.accountModal.walletAddressInput);
  ...
  initialSetup: async ({ secretWordsOrPrivateKey, network, password }) => {
    const isCustomNetwork = process.env.NETWORK_NAME && process.env.RPC_URL && process.env.CHAIN_ID;
