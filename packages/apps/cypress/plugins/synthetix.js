...
module.exports = {
  settle: async ({ asset, walletAddress, privateKey }) => {
    ...
    const signer = new SynthetixJs.signers.PrivateKey(undefined, networkId, `0x${privateKey}`);
    const snxjs = new SynthetixJs({ signer, networkId });

    const maxEntriesInQueue = await getMaxEntriesInQueue(networkName);
    const currentLengthOfEntries = await getLengthOfEntries(networkName, walletAddress, assetAsBytes32);
    ...
    if (currentLengthOfEntries >= maxEntriesInQueue - 3) {
      ...
      console.log(`Current length of entries in queue is ${currentLengthOfEntries}, no need to settle yet.`);
      return false;
    }
...
async function getLengthOfEntries(networkName, walletAddress, assetAsBytes32) {
  const snxjs = synthetix({ network: networkName });
  const getLengthOfEntries = await snxjs.contracts.ExchangeState.getLengthOfEntries(walletAddress, assetAsBytes32);
...
async function getMaxSecsLeftInWaitingPeriod(networkName, walletAddress, assetAsBytes32) {
  const snxjs = synthetix({ network: networkName });
